...
tablet_setup() {
  # echo "Configuring mode and buttons..."
  PEN="$(xsetwacom list devices | grep -Eo -m 1 '[a-zA-Z0-9./?=_%:-\ ]*stylus')"
  xsetwacom set "$PEN" mode Absolute
  # xsetwacom set "$PEN" Button 1
  # xsetwacom set "$PEN" Button 2
  PAD="$(xsetwacom list devices | grep -Eo -m 1 '[a-zA-Z0-9./?=_%:-\ ]*pad')"
  xsetwacom set "$PAD" Button 1 "key +ctrl i -ctrl"
  xsetwacom set "$PAD" Button 2 "key +ctrl e -ctrl"
  # xsetwacom set "$PAD" Button 2 key +ctrl
  # xsetwacom set "$PAD" Button 3 key +ctrl +shift z -shift -ctrl
